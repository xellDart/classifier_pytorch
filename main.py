...
ap.add_argument("-m", "--model", type=str, default="MobileNetV3_Small",
        ...
        for phase in ["train", "val"]:
            ...
            for inputs, labels in dataloaders[phase]:
                ...
                with t.autograd.set_grad_enabled(phase == "train"):
                    # bsize * 2 , because it is a binary classification
...
if __name__ == '__main__':
    ...
    #optimizer = t.optim.AdamW(model.parameters(), lr=0.001, betas=(
    #    0.9, 0.999), eps=1e-08, weight_decay=0.01, amsgrad=False)
    optimizer = t.optim.SGD(model.parameters(), lr=1e-1, momentum=0.9, weight_decay=1e-5)
    if train_sign:
        model, val_logs = train_model(
            model, dataloader, loss_fn, optimizer, epochs)
        # store the model
        import time
